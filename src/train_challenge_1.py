...
# %% Dataset wrapper with normalization & augmentation [code]
import numpy as np

class StandardizedWindowDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Per-sample mean/std normalization (matches submission.py)
        mean = X.mean(dim=-1, keepdim=True)
        std = X.std(dim=-1, keepdim=True).clamp_min(1e-6)
        X = (X - mean) / std

        if self.augment:
            # Small time shift augmentation
...
batch_size = 64
num_workers = 0  # to avoid multiprocessing issues on certain OS

train_ds = StandardizedWindowDataset(train_set, augment=True)
valid_ds = StandardizedWindowDataset(valid_set, augment=False)
test_ds = StandardizedWindowDataset(test_set, augment=False)
...
print(f"Test RMSE: {test_rmse:.6f}, Test Loss: {test_loss:.6f}")
